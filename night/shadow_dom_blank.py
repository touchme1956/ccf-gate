...
■ 案
  ...
  drop  : **dom を丸ごと撤去**（全社が4本式＝今の空欄社と同じ扱い）
          ⚠ 実測: **V(dom=85) が投下可から落ち ASML(dom=100) が次点へ下がる**一方、
          **dom=50 の55社が +2.2pt（最大+15.0）得をする**＝**測定に成功した社だけが罰される**。
          判定圏の四関門通過は 9→12社 で**門が緩くなる**。全文 docs/CLAUDE_ARCHIVE.md
...
def patch(v):
    """v=数値なら空欄をその値で埋める。v=="drop" なら dom を丸ごと落とす（全社4本式）。"""
    if v == "drop":
        return "  d=Object.assign({},d,{dom:null});\n" + ANCH
