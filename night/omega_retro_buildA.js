 ...
 * 出力:   out/omega_retro_A_{2013,2015}.json
 */
'use strict';
const fs = require('fs');
...
const V = {
  2013: { pil: 'out/retro_moat_pillars_2013.json',
          ...
          mach: 'out/omega_retro_machine_2013.json', judg: 'out/omega_retro_judg_2013.json' },
...
for (const yr of Object.keys(V)) {
  ...
  // ★案A: p1/p2/p4 を門の刻みで機械導出したもの（night/omega_retro_judg.py）。
  //   f1 は roiic の被覆(11%/53%)と WACC 不在で埋められない＝空欄のまま（限界として報告する）。
  const J = JSON.parse(fs.readFileSync(path.join(ROOT, v.judg), 'utf8')).items || {};
