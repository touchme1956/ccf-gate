...
def main():
    ...
    for nm, pool in (("2018年 全母集団", rows), ("2018年 質実証プール＝門が当てる場所", qp)):
        ...
        res[nm + "/PER"] = cut(pool, b, lambda r: r["per"], (40, 50, 60, 70, 80), "生PER")
        res[nm + "/ratio"] = cut(pool, b, lambda r: r["ratio"], (2.0, 2.5, 3.0, 3.5, 4.0),
                                 "現PER÷fairPER（成長で正規化）")
        print()
    ...
    for yr, pf, rf in (("2013", "retro_per_2013_all.json", "retro_returns_2013_all.json"),
        ...
        res[yr] = cut(g, b, lambda r: r["per"], (40, 50, 60), "生PER")
        print()
