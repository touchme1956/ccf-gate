...
やること:
  score_all.js の四段関門の**価格条件の項だけ**を差し替えて回し、**必ず元へ戻す**。
  合成点上位10社の枠は門の ccfAllocTop（単一実装）をそのまま使う＝二つ目の検査器を作らない（v9.9.65の掟）。
...
def main():
    ...
    if ANCHOR not in src:
        raise SystemExit("錨が見つからない（score_all.js の四段関門の行が変わった？）")
