...
■ 何を問うか
  ...
  **だが「最小値」は定義上ほとんど何も落とさない。** 実測（2026-08-11）:
  ...
  門はこれを **11.9 / 0.64 / 1.8 へ丸め上げている**。その結果
  **線を作った当の3社が自分の線で落ちる**（COHR −0.01 / WST −0.001 / CW −0.04）。
...
def main():
    ...
    print('  ・**直すべきは丸め**。実測の最小値は 営利率11.89 / FCF転換0.639 / cagr1.76 なのに')
    print('    門は 11.9 / 0.64 / 1.8 へ**丸め上げて**おり、線を作った当の3社（COHR/WST/CW）が')
    print('    自分の線で落ちる。**床のつもりが勝者に食い込んでいる**（規約の改定はルール1の領分）')
