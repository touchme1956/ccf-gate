...
■ 初回実測（2026-07-29）
  ...
  p1: **規約そのものが高ROIC企業で機能しない**ことが判った。刻みは σ<3pt→90 / <6pt→80 /
      σ8pt級→65 / 二桁→50 と**絶対の百分点**で切るが、これは ROIC が 10-25% の帯を想定した目盛りで、
      ROIC が 50-130% ある会社では σ が必ず二桁になる。実測（食い違い105社のサンプル25社）:
        ・**ROIC平均が40%以上の社が18/25**——この帯では絶対σ刻みは自動的に 50 を返す
        ・台帳p1の中央値 80 に対し、絶対σ刻みの中央値は **50**。一致率は **0/25**
        ・V: ROIC 76/102/114/138/128% → σ21.5pt → 刻み50。だが実態は「一貫して桁外れに高い」
        ・OTIS: 自己資本が負でICが縮退し ROIC 3108% → σ1190pt（そもそも測れていない）
      対して**変動係数(σ÷平均)**で切ると一致率は 32% まで上がる（<15%→90 /<30%→80 /<50%→65）。
      → **これは台帳105社の誤りではなく、ルーブリックの目盛りの誤りである。**
      審査官が置いた75-85のほうが実態に近い。**刻みの変更は採点基準なのでユーザー明示指示が要る**
      （CLAUDE.md「絶対のルール1」／較正は calibration_check.py の年次レビュー事項）。
      それまでは p1 の食い違いを作業リストとして扱わないこと——直しに行くと台帳を壊す。
      根拠を刻めた16社は、ROICが常識帯にあり絶対σ刻みが機能している社。
"""
import json
import os
import statistics
...
def p1_of(sigma):
    """審査プロトコルの刻み: σ<3pt→90 / <6pt→80 / σ8pt級→65 / 二桁→50"""
    ...
    for fn in files:
        ...
        if do_p1 and not t[0].isdigit() and isinstance(d.get("p1"), (int, float)) and not ev.get("p1"):
            ...
            if ser is None:
                ...
                if abs(d["p1"] - got) < 0.5:
                    ev["p1"] = (f"【2026-07-29 機械算出】ROIC安定性＝のれん・無形除外ROIC{len(ser)}年系列の標準偏差。"
                                f"系列: {txt} → **σ={sd:.1f}pt** → 刻み(σ<3→90 / <6→80 / <10→65 / 二桁→50)で **{got}**。"
