...
# 実行: python3 night/scan_irr85_universe.py [--days 1200]
...
def fetch(q, frm=0):
    u = ("https://efts.sec.gov/LATEST/search-index?q=" + urllib.parse.quote(f'"{q}"')
         + f"&forms=10-K,20-F&startdt={START}&enddt={END}&from={frm}")
    for a in range(4):
        try:
...
json.dump({"generated": END.isoformat(), "window": f"{START}..{END}",
           "note": "全上場企業のEDGAR全文検索。機構語(STRONG)の有無で候補を仕分ける。"
                   "10-K/20-Fのみ＝日本株は対象外（EDINETは別経路）",
