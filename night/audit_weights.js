...
// 名目の実効ウェイト（門のソースの重みを階層どおり掛けた解析値）
//   Ω  = gm([Q .35, sustain .30, F .22, moat .13])
//   Q  = gm([P .40, now .35, F .25])      ← **F がここにも居る**＝二経路
//   P  = gm([p1 .30, p2 .25, p3 .25, p4 .20])
//   sustain = gm([pm .475, pr .525]) ／ pm = .70*moat + .30*営業利益率（線形）
const FW = 0.22 + 0.35 * 0.25;              // F の実効ウェイト（直接 + Q経由）
const MW = 0.13 + 0.30 * 0.475 * 0.70;      // 堀の実効ウェイト（直接 + pm経由）
const PW = 0.35 * 0.40;                     // P の実効ウェイト
const NOMINAL = {
