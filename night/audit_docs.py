...
def check(h, f):
    ...
    hist = re.compile(r'v9\.9\.\d+.{0,80}?(?:にした|した——|へ改定|から連続|旧・|旧式|当時|実測)')
    hits = [x for x in prose_hits(r'門X4条件|門X（✦超未来の門・裁きⅠ）の4条件|門Xの4条件')
            if not hist.search(x[1])]
    if hits:
    ...
    # ⑨ 堀が「格下げ・保険」と説明されているのに、実装では三関門の一つ（2026-08-07新設）
