...
for (const f of fs.readdirSync(path.join(ROOT, 'out'))) {
  ...
  rows.push({ t, nm, jp, s, tier: r.tierShort,
              ...
              //   irr=85 ∧ 歴史の継続組の下限（営利率11.89 / FCF転換0.639 / cagr1.76・v9.9.129で丸め上げを撤回）∧ nde≤4 なら
...
//   ★席の数は v9.9.171(ユーザー明示指示「買付は5社に縛って」)で 10→5。**数字は門の CCF_SEATS 一箇所**で、
//   ここは引数を省いて呼び、表示に要る社数は戻り値の sel.seats から読む（書き写さない）。
//   投下可＝四関門∧席順の上位 sel.seats 社（v9.9.98でE[r]項を外し・v9.9.100でirr=85を先頭へ）。判定は門の ccfAllocTop（単一実装＝Ⅵ・盤・snapと同一・v9.9.65の掟）。
//   四段通過だが席から溢れた社は quali=true / buy=false ＝🔵次点（買わないが資格は保持）。
let ALLOC_SEMI_N = 0, ALLOC_SEMI_PCT = 0;   // v9.9.145: 上限は撤去。集中は測って出し続ける
let ALLOC_SEATS = 0;                        // v9.9.171: 席の数（門の CCF_SEATS）を表示へ配る
{
  const four = rows.filter(r => r.buy);
  const sel = ccfAllocTop(four);   // v9.9.171: 席の数は門の CCF_SEATS（書き写さない）
  ALLOC_SEMI_N = sel.semiN || 0; ALLOC_SEMI_PCT = sel.semiPct || 0; ALLOC_SEATS = sel.seats || 0;
...
for (const r of q75) {
  ...
  console.log(`  ${(r.nm.slice(0, 24) + (r.frame85 && r.s < 75 ? '【別枠85】' : '')).padEnd(26)} Ω${r.s.toFixed(1).padStart(5)}  堀${moat}${r.moatOK ? '✓' : '✗'}`
    ...
    + `  点${aud}`
    + `  ${r.buy ? '🟢投下可' : r.quali ? `🔵次点(${ALLOC_SEATS + 1}位以下)` : blockers(r).join('＋')}  出口=${r.exit}`);
}
...
console.log(`\n🟢投下可(四関門∧irr=85優先→Ω順の上位${ALLOC_SEATS}社／半導体上限は v9.9.145 で撤去) ${buy.length}社`
  + `　日本株${buy.filter(x => x.jp).length}／米国等${buy.filter(x => !x.jp).length}`
  + `\n  ${buy.map(x => x.nm.split(/\s/)[0]).join(' ') || '(なし)'}`);
if (nextUp.length) console.log(`🔵次点(四関門通過・席順${ALLOC_SEATS + 1}位以下＝買わない) ${nextUp.length}社\n  ${nextUp.map(x => x.nm.split(/\s/)[0]).join(' ')}`);
...
if (ALLOC_SEMI_N >= 3)
  console.log(`◇半導体連鎖の集中(注記・席の選定には効かない) ${ALLOC_SEMI_N}社＝席の${ALLOC_SEMI_PCT}%`
    + ` ／ 上限は2026-08-14に撤去（ユーザー明示指示）。1銘柄8%は守るが束の相関は誰も止めない`);
