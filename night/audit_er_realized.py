...
// v9.9.88: 第五の枠（席順＝irr=85優先→Ω順・v9.9.100。席の数は門の CCF_SEATS＝v9.9.171 で5）——門の ccfAllocTop をそのまま使う（単一実装・v9.9.65の掟）。
// quali=四段側の資格 / buy=枠内。観測の意味変化は er_ledger の basis_changes に記録済み
if(typeof ccfAllocTop==='function'){
  const four=out.filter(o=>o.buy);
  const sel=ccfAllocTop(four.map(o=>({t:o.t,s:o.omega,xEr:o.er})));   // v9.9.171: 席の数は門の CCF_SEATS
