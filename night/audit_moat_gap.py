...
# -*- coding: utf-8 -*-
"""
night/audit_moat_gap.py — 堀のふるい(絶対MOAT指数75+)に**あと何が足りないか**を刻み単位で出す（2026-07-29新設）

audit_moat.py との違い:
  audit_moat は「根拠の質」を見る（測っているか）。こちらは「**あと一段上げれば通るか**」を見る。
  再監査は原本を読む重い作業なので、**読んでも合否が動かない社を先に外す**のがこの道具の仕事。

なぜ要るか（初回実測 2026-07-29）:
  Ω75+ は29社あるのに、三段関門（Ω75+ ∧ 門X4条件 ∧ 堀75+）を通るのは3社しかない。
...
使い方:
  python3 night/audit_moat_gap.py          Ω75+ で堀<75 の全社
  python3 night/audit_moat_gap.py --all    Ω・門Xを問わず堀<75 の全社
...
def main():
    ...
    for t, r, d, cur, single, hopeless, fresh in rows:
        ...
        if hopeless:
            note = "**全本を満点にしても75に届かない＝原本を読んでも動かない**"
        elif not single:
