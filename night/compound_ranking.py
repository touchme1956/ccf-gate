...
def main():
    ...
    # ⚠ 社数は焼き付けない（2026-09-19: 見出しが14のまま表は15行あった＝「数字を書き写した箇所は
    #    必ず陳腐化する」の実例。母集団は審査が進むと動くので rows から数える）
    print(f'■ irr=85 の{len(rows)}社を「今後も高い複利を出す可能性が高い順」に並べる')
    ...
    print(f'  ・**この{len(rows)}社が母集団のすべて**——2026-08-08 に EDGAR全文（10-K 403社＋20-F製造業38社）を')
    print('    読んで**新しい irr=85 はゼロ**だった。外側に同じ型は（今のところ）居ない。')
    print('  ・⚠**実績の窓は 2018→2026 の一つだけ**で、半導体・AI相場に極端に有利。')
    _mx = max((r for r in rows if r['self_ret'] is not None and r['self_rev'] is not None),
              key=lambda r: r['self_ret'] - r['self_rev'], default=None)
    if _mx:  # 名指しも焼き付けない——残差が最大の社は窓と価格で入れ替わる
        print(f"    {_mx['t']} の {_mx['self_ret']:+.1f}% は残差 {_mx['self_ret']-_mx['self_rev']:+.1f}pt（＝再評価）で、"
              f"事業が稼いだ {_mx['self_rev']:.1f}% とは別物として読む")
