...
こちらは「D1 の手続きでは構造的に扱えなかった3つの角」を測る。
 ...
 (c) **退場社を戻した母集団**（2013ビンテージ・price-only）
     ...
     **最初から最後まで price-only の中だけで完結**させ、配当込みの数字とは並べるだけで割らない。
     出すのは点推定ではなく**挟み込み**（打ち切り714社〔2026-09-23 退場日の是正後。旧762社〕の返り値は不明なので、
...
・(c) の打ち切り714社（2026-09-23 退場日の是正後・旧762社）のうち非M&Aは限られ、
  **返り値そのものが不明**な社は挟み込みにしか入らない。
...
def _c_reading(dl):
    """(c) の読み。打ち切りの社数と挟み込みの幅は出力から取る（固定文にしない）。"""
    n_cens = dl["universe"]["kind"].get("打ち切り")
    det = sum(1 for gs in dl["group_brackets"].values() for g in gs if g.get("sign_determined"))
    wd = {pool: [g["bracket_width"] for g in gs if g.get("bracket_width") is not None]
          for pool, gs in dl["group_brackets"].items()}
    ws = " / ".join(f"{pool} {min(v):.2f}-{max(v):.2f}" for pool, v in wd.items() if v)
    head = ("**群の差の符号が決まった群はゼロ。** " if det == 0
            else f"**群の差の符号が決まった群は {det}。** ")
    return (head + f"打ち切り{n_cens}社の返り値が不明なので挟み込みの幅（{ws}）が、"
...
doc = {
    ...
    "regenerated": __import__("datetime").date.today().isoformat(),
    ...
    "findings_summary": {
        ...
        "(c)退場社を戻した母集団": {
            ...
            # ★2026-09-23: 打ち切りの社数と幅を**数字から組む**（旧版は『762社・全社 0.82-1.01 /
            #   質実証 0.46-0.50』の固定文で、退場日の是正〔out/retro_exit_fix_2013.json〕の後も残った）
            "reading": _c_reading(delisted),
        },
    },
    ...
    "limitations": [
        ...
        f"(c) は price-only・2013の1ビンテージ・打ち切り{delisted['universe']['kind'].get('打ち切り')}社。"
