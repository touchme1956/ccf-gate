...
def norm(s):
    ...
    #   （2026-08-20の実害）。PDFの行折り返しが語の途中に入るので、原本は
    #   `…欠かせないシステム・サービスとし\nて、当連結会計年度で…` のように割れる。
    #   引用側は割れていないので**一字も違わないのに逐語照合が落ちる**——
    #   実測 3649・6273 は空白を潰せば原本に完全一致した（＝引用は本物）。
    #   `**` の取りこぼし(ST・2026-08-11)／U+2011(RMD・2026-08-19)／
    #   XBRLタグ割れの `$ 678`(MSFT・2026-08-19)と**同じ族の4例目**で、
    #   どれも「機構文が消えた」という**危ない側へ片寄って**壊れる。
    ...
    s = re.sub(r'\s+([,.;:)])', r'\1', s)
    s = re.sub(r'([(])\s+', r'\1', s)
    # ⚠**通貨記号・%と数字の間の空白を畳む**（2026-08-19の実害）——
    #   XBRL のインライン・タグは数字だけを別要素に包むので、タグを剥がすと
    #   `$678 billion` が **`$ 678 billion`**、`30%` が **`30 %`** になって出てくる。
    #   実測: MSFT の Note 12（commercial RPO $678十億・2.3年）がこれで一致 **47%** まで落ち、
    #   「数字が更新された」側に見えていた（実際は一字も違わない）。
    #   **数字は機構文をいちばん強くする部分なので、ここを取りこぼすと最良の証拠から先に誤検出する。**
    #   `**` の取りこぼし(2026-08-11 ST)・U+2011 の取りこぼし(2026-08-19 RMD)と**同じ族の欠陥**。
