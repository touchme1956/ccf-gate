...
(function () {
  ...
  var EXACT = ['pf:portfolio', 'pf:weights', 'pf:sold', 'pf:monthly_total', 'pf:monthly', 'pf:monthly_net', 'g7ignite:map'];
  var PREFIX = ['g7log:'];
  ...
  var ORDER = ['pf:portfolio', 'pf:sold', 'pf:weights', 'pf:monthly_total', 'pf:monthly', 'pf:monthly_net', 'g7ignite:map'];
  ...
  /* ⚠ **素の文字列比較では一度も一致しない**。決定のファイルに機械の書き戻しが混ざっているから。
     推測せず実測した（2026-08-12・素の読み込み直後に state.json と突き合わせ）——食い違うのは
       ...
       pf:sold / pf:monthly_total / pf:monthly / pf:monthly_net / g7ignite:map … **完全一致**
