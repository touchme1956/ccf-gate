# =====================================================================
# CCF Ω Final Gate — 第0の門 統合版 v8.2（1セル・1実行で全工程）
#
...
pool  = sorted([r for r in keep if not r["equity_neg"]] + neg7, key=lambda r: -r["pt"])
...
print(f"\n■ 自己資本マイナス枠(6点のみ・7点{len(neg7)}社は⚑合流): {len(negeq)} 社 / backlog: {len(backlog)} 社")
...
for t in sorted(WATCH):
    ...
    st = ("待ち行列⚑負資本" if t in qs and r and r.get("neg_flag")
          ...
          else "★のれん椅子" if t in gs else "自己資本マイナス枠" if t in ns
