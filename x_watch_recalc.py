...
# -*- coding: utf-8 -*-
"""
x_watch_recalc.py — X監視表(gate1_x_watch.json)の開通ラインを E[r]=0 の線で再計算する。
※ **v9.9.98(2026-08-07)以降、この線は門Ωの合否ではない**——門Ωの関門は Ω75+ ∧ 堀70+ ∧ データ健全の
  四関門になり、価格は合否に一切使われない。開通ラインは**階段指値の先段の目安**として生き続ける
  （深段は fair線＝倍率の重力ゼロ）。式は従来どおり index.html の ccfXJudge と同一（v9.9.65の掟）。
