...
function ccfIconFace(nm){
  ...
  // v9.9.153（ユーザー指示「入れて」）: **モノクロのロゴは自分の明度で灰に染める**。
  //   k は night/logo_colors.py が「有彩色が採れなかったロゴ」の**明度の中央値**から作った灰。
  //   黒・白・灰はそのロゴが現に持っている色なので、これは v9.9.105 が禁じた
  //   **ハッシュ色（そのロゴと何の関係も無い色）で染めること**には当たらない。
  //   ⚠ 灰は saturate(1.35) の恩恵を受けられない（彩度0を1.35倍しても0）ので同じ不透明度でも
  //     有彩色より弱く見える。**不透明度は変えない**——「もう少し薄く」を二度受けて較正した数字だから。
  //     代わりに night/logo_colors.py 側で**灰の明度を地から離す**（帯 [0.40,0.60]・実測つき）。
  //     実測の見え方（行中央と地の差／明・暗テーマ）: 有彩 67/40 ・ 灰 21〜32/18〜28 ・ 色なし 0/0。
  //   ⚠ 変数名 mono は上でモノグラムの2文字に使っているので、こちらは isGrey とする
  const kg=(m&&m.k)||null, isGrey=!c&&!!kg;
  const cs=(m&&m.cs)||(c?[c]:(kg?[kg]:[]));
  const PX=x=>[1,3,5].map(i=>parseInt(x.substr(i,2),16));
  const A=(x,a)=>`rgba(${PX(x).join(',')},${a})`;
  const glow=c||kg||(ext?'rgba(200,190,170,.5)':`hsl(${hue} 62% 52%)`);
