# =====================================================================
# CCF Ω Final Gate — 第0の門 統合版 v8.1（1セル・1実行で全工程）
#
...
sound = sorted([r for r in keep if not r["equity_neg"]], key=lambda r: -r["pt"])
negeq = sorted([r for r in keep if r["equity_neg"]], key=lambda r: -r["pt"])
queue, backlog = sound[:TOP_N], sound[TOP_N:]
...
print(f"\n■ 自己資本マイナス枠: {len(negeq)} 社 / backlog: {len(backlog)} 社")
...
for t in sorted(WATCH):
    r = BYT.get(t)
    st = ("待ち行列" if t in qs else "★椅子" if t in cs else "自己資本マイナス枠" if t in ns
